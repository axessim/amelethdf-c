...
char AH5_read_ft_singleinteger (hid_t file_id, const char *path, AH5_singleinteger_t *singleinteger)
{
  ...
  if (AH5_read_int_attr(file_id, path, AH5_A_VALUE, &(singleinteger->value)))
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(singleinteger->opt_attrs), mandatory,
...
char AH5_read_ft_singlereal (hid_t file_id, const char *path, AH5_singlereal_t *singlereal)
{
  ...
  if (AH5_read_flt_attr(file_id, path, AH5_A_VALUE, &(singlereal->value)))
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(singlereal->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
  {
...
char AH5_read_ft_singlecomplex (hid_t file_id, const char *path, AH5_singlecomplex_t *singlecomplex)
{
  ...
  if (AH5_read_cpx_attr(file_id, path, AH5_A_VALUE, &(singlecomplex->value)))
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(singlecomplex->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
  {
...
char AH5_read_ft_singlestring (hid_t file_id, const char *path, AH5_singlestring_t *singlestring)
{
  ...
  if(AH5_read_str_attr(file_id, path, AH5_A_VALUE, &(singlestring->value)))
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(singlestring->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
  {
...
char AH5_read_ft_vector (hid_t file_id, const char *path, AH5_vector_t *vector)
{
  ...
  if (rdata)
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(vector->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
  {
...
char AH5_read_ft_linearlistofreal1 (hid_t file_id, const char *path,
                                    AH5_linearlistofreal1_t *linearlistofreal1)
{
  ...
  if (H5Aexists_by_name(file_id, path, AH5_A_FIRST, H5P_DEFAULT) > 0
      && H5Aexists_by_name(file_id, path, AH5_A_LAST,
                           H5P_DEFAULT) > 0)  // interval doesn't have AH5_A_NUMBER_OF_VALUES
    if (H5LTget_attribute_float(file_id, path, AH5_A_FIRST, &(linearlistofreal1->first)) >= 0
        && H5LTget_attribute_float(file_id, path, AH5_A_LAST, &(linearlistofreal1->last)) >= 0)
      rdata = AH5_TRUE;
  if (H5Aexists_by_name(file_id, path, AH5_A_NUMBER_OF_VALUES, H5P_DEFAULT) > 0)
    if (H5LTget_attribute_int(file_id, path, AH5_A_NUMBER_OF_VALUES,
                              &(linearlistofreal1->number_of_values)) < 0)  // AH5_A_NUMBER_OF_VALUES is present -> must be read
      rdata = AH5_FALSE;
  if (rdata)
  {
    linearlistofreal1->path = strdup(path);
    AH5_read_opt_attrs(file_id, path, &(linearlistofreal1->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
...
char AH5_read_ft_linearlistofreal2 (hid_t file_id, const char *path,
                                    AH5_linearlistofreal2_t *linearlistofreal2)
{
  ...
  if (H5Aexists_by_name(file_id, path, AH5_A_FIRST, H5P_DEFAULT) > 0
    ...
    if (H5LTget_attribute_float(file_id, path, AH5_A_FIRST, &(linearlistofreal2->first)) >= 0
        ...
        && H5LTget_attribute_int(file_id, path, AH5_A_NUMBER_OF_VALUES,
                                 &(linearlistofreal2->number_of_values)) >= 0)
      rdata = AH5_TRUE;
  if (rdata)
  {
    linearlistofreal2->path = strdup(path);
    AH5_read_opt_attrs(file_id, path, &(linearlistofreal2->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
    printf("***** ERROR: Cannot read mandatory attributes in \"%s\". *****\n\n", path);
...
char AH5_read_ft_logarithmlistofreal (hid_t file_id, const char *path,
                                      AH5_logarithmlistofreal_t *logarithmlistofreal)
{
  ...
  if (rdata)
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(logarithmlistofreal->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
    printf("***** ERROR: Cannot read mandatory attributes in \"%s\". *****\n\n", path);
...
char AH5_read_ft_perdecadelistofreal (hid_t file_id, const char *path,
                                      AH5_perdecadelistofreal_t *perdecadelistofreal)
{
  ...
  if (H5Aexists_by_name(file_id, path, AH5_A_FIRST, H5P_DEFAULT) > 0
    ...
    if (H5LTget_attribute_float(file_id, path, AH5_A_FIRST, &(perdecadelistofreal->first)) >= 0
        && H5LTget_attribute_int(file_id, path, AH5_A_NUMBER_OF_DECADES,
                                 &(perdecadelistofreal->number_of_decades)) >= 0
        && H5LTget_attribute_int(file_id, path, AH5_A_NUMBER_OF_VALUES_PER_DECADE,
                                 &(perdecadelistofreal->number_of_values_per_decade)) >= 0)
      rdata = AH5_TRUE;
  if (rdata)
  {
    perdecadelistofreal->path = strdup(path);
    AH5_read_opt_attrs(file_id, path, &(perdecadelistofreal->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
    printf("***** ERROR: Cannot read mandatory attributes in \"%s\". *****\n\n", path);
...
char AH5_read_ft_linearlistofinteger2 (hid_t file_id, const char *path,
                                       AH5_linearlistofinteger2_t *linearlistofinteger2)
{
  ...
  if (rdata)
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(linearlistofinteger2->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
    printf("***** ERROR: Cannot read mandatory attributes in \"%s\". *****\n\n", path);
...
char AH5_read_ft_rationalfunction (hid_t file_id, const char *path,
                                   AH5_rationalfunction_t *rationalfunction)
{
    ...
    if (nfields == 4 && rationalfunction->nb_types > 0)
    {
      ...
      if (H5TBget_field_info(file_id, path, field_names, field_sizes, field_offsets, &type_size) >= 0)
        if (strcmp(field_names[0], AH5_F_TYPE) == 0 && strcmp(field_names[1], AH5_F_A) == 0
            && strcmp(field_names[2], AH5_F_B) == 0 && strcmp(field_names[3], AH5_F_F) == 0)
        {
          ...
          if (H5TBread_fields_index(file_id, path, 1, &type, 0, rationalfunction->nb_types, sizeof(int),
                                    field_offsets, field_sizes, rationalfunction->types) >= 0
              && H5TBread_fields_index(file_id, path, 1, &a, 0, rationalfunction->nb_types, sizeof(float),
                                       field_offsets, field_sizes, rationalfunction->a) >= 0
              && H5TBread_fields_index(file_id, path, 1, &b, 0, rationalfunction->nb_types, sizeof(float),
                                       field_offsets, field_sizes, rationalfunction->b) >= 0
              && H5TBread_fields_index(file_id, path, 1, &f, 0, rationalfunction->nb_types, sizeof(float),
                                       field_offsets, field_sizes, rationalfunction->f) >= 0)
            rdata = AH5_TRUE;
          else
          {
  ...
  if (rdata)
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(rationalfunction->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
...
char AH5_read_ft_generalrationalfunction (hid_t file_id, const char *path,
    AH5_generalrationalfunction_t *generalrationalfunction)
{
  ...
  if (H5LTget_dataset_ndims(file_id, path, &nb_dims) >= 0)
    if (nb_dims == 2)
      if (H5LTget_dataset_info(file_id, path, dims, &type_class, &length) >= 0)
        ...
        {
          generalrationalfunction->numerator = (AH5_complex_t *) malloc((size_t) dims[0] * sizeof(
                                                 AH5_complex_t));
          generalrationalfunction->denominator = (AH5_complex_t *) malloc((size_t) dims[0] * sizeof(
              AH5_complex_t));
  ...
  if (rdata)
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(generalrationalfunction->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
...
char AH5_read_ft_rational (hid_t file_id, const char *path, AH5_rational_t *rational)
{
  ...
  if (rdata)
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(rational->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
    printf("***** ERROR: Cannot read dataset \"%s\". *****\n\n", path2);
...
char AH5_read_ft_dataset (hid_t file_id, const char *path, AH5_dataset_t *dataset)
{
  ...
  if (rdata)
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(dataset->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
    AH5_print_err_dset("", path);
...
char AH5_read_ft_arrayset (hid_t file_id, const char *path, AH5_arrayset_t *arrayset)
{
  ...
  if (rdata)
  {
    ...
    AH5_read_opt_attrs(file_id, path, &(arrayset->opt_attrs), mandatory,
                       sizeof(mandatory)/AH5_ATTR_LENGTH);
  }
  else
    ...
    if (AH5_read_str_attr(file_id, path, AH5_A_FLOATING_TYPE, &buf))
    {
      ...
      else if (strcmp(buf, AH5_V_GENERAL_RATIONAL_FUNCTION) == 0)
      {
        ...
        if (AH5_read_ft_generalrationalfunction(file_id, path,
                                                &(floatingtype->data.generalrationalfunction)))
          rdata = AH5_TRUE;
      }
...
char AH5_init_floatingtype (hid_t file_id, const char *path)
{
  ...
  if (path != NULL && !AH5_path_valid(file_id, path))
  {
    ...
    if (AH5_path_valid(file_id, path2))
      success = AH5_TRUE;
    else if (strcmp(path2, AH5_C_FLOATING_TYPE) == 0)
...
void AH5_print_ft_vector (const AH5_vector_t *vector, int space)
{
  ...
  printf("%*s-%s [%lu]: {", space, "", AH5_get_name_from_path(vector->path),
         (long unsigned) vector->nb_values);
  switch (vector->type_class)
  {
  case H5T_INTEGER:
  ...
  case H5T_COMPOUND:
    ...
    printf("%g%+gi}\n", creal(vector->values.c[vector->nb_values - 1]),
           cimag(vector->values.c[vector->nb_values - 1]));
    break;
  case H5T_STRING:
    for (i = 0; i < vector->nb_values - 1; i++)
...
{
  printf("%*s-%s: %s=%g, %s=%g, %s=%i\n", space, "",AH5_get_name_from_path(linearlistofreal1->path),
         AH5_A_FIRST, linearlistofreal1->first, AH5_A_LAST, linearlistofreal1->last, AH5_A_NUMBER_OF_VALUES,
         linearlistofreal1->number_of_values);
...
{
  printf("%*s-%s: %s=%g, %s=%g, %s=%i\n", space, "",AH5_get_name_from_path(linearlistofreal2->path),
         AH5_A_FIRST, linearlistofreal2->first, AH5_A_STEP, linearlistofreal2->step, AH5_A_NUMBER_OF_VALUES,
         linearlistofreal2->number_of_values);
  AH5_print_opt_attrs(&(linearlistofreal2->opt_attrs), space + 3);
}


// Print logarithmListOfReal
void AH5_print_ft_logarithmlistofreal (const AH5_logarithmlistofreal_t *logarithmlistofreal,
                                       int space)
{
  printf("%*s-%s: %s=%g, %s=%g, %s=%i\n", space, "",
         AH5_get_name_from_path(logarithmlistofreal->path), AH5_A_FIRST, logarithmlistofreal->first,
         AH5_A_LAST, logarithmlistofreal->last, AH5_A_NUMBER_OF_VALUES,
         logarithmlistofreal->number_of_values);
  AH5_print_opt_attrs(&(logarithmlistofreal->opt_attrs), space + 3);
}


// Print perDecadeListOfReal
void AH5_print_ft_perdecadelistofreal (const AH5_perdecadelistofreal_t *perdecadelistofreal,
                                       int space)
{
  printf("%*s-%s: %s=%g, %s=%i, %s=%i\n", space, "",
         AH5_get_name_from_path(perdecadelistofreal->path), AH5_A_FIRST, perdecadelistofreal->first,
         AH5_A_NUMBER_OF_DECADES, perdecadelistofreal->number_of_decades, AH5_A_NUMBER_OF_VALUES_PER_DECADE,
         perdecadelistofreal->number_of_values_per_decade);
  AH5_print_opt_attrs(&(perdecadelistofreal->opt_attrs), space + 3);
}


// Print linearListOfInteger2
void AH5_print_ft_linearlistofinteger2 (const AH5_linearlistofinteger2_t *linearlistofinteger2,
                                        int space)
{
  printf("%*s-%s: %s=%i, %s=%i, %s=%i\n", space, "",
         AH5_get_name_from_path(linearlistofinteger2->path), AH5_A_FIRST, linearlistofinteger2->first,
         AH5_A_STEP, linearlistofinteger2->step, AH5_A_NUMBER_OF_VALUES,
         linearlistofinteger2->number_of_values);
...
void AH5_print_ft_rationalfunction (const AH5_rationalfunction_t *rationalfunction, int space)
{
  ...
  printf("%*s-%s [%lu]: ", space, "", AH5_get_name_from_path(rationalfunction->path),
         (long unsigned) rationalfunction->nb_types);
  for (i = 0; i < rationalfunction->nb_types - 1; i++)
  {
    printf("type%i=%g|%g|%g, ", rationalfunction->types[i], rationalfunction->a[i],
           rationalfunction->b[i], rationalfunction->f[i]);
  }
  printf("type%i=%g|%g|%g\n", rationalfunction->types[rationalfunction->nb_types - 1],
         rationalfunction->a[rationalfunction->nb_types - 1],
         rationalfunction->b[rationalfunction->nb_types - 1],
         rationalfunction->f[rationalfunction->nb_types - 1]);
  AH5_print_opt_attrs(&(rationalfunction->opt_attrs), space + 3);
}


// Print generalRationalFunction
void AH5_print_ft_generalrationalfunction (const AH5_generalrationalfunction_t
    *generalrationalfunction, int space)
{
  hsize_t i;

  printf("%*s-%s [%lu]:\n", space, "", AH5_get_name_from_path(generalrationalfunction->path),
         (long unsigned) generalrationalfunction->nb_degrees);
  for (i = 0; i < generalrationalfunction->nb_degrees; i++)
    printf("%*s-degree %lu: numerator=%g%+gi, denominator=%g%+gi\n", space + 3, "", (long unsigned) i,
           creal(generalrationalfunction->numerator[i]), cimag(generalrationalfunction->numerator[i]),
           creal(generalrationalfunction->denominator[i]), cimag(generalrationalfunction->denominator[i]));
...
void AH5_print_ft_rational (const AH5_rational_t *rational, int space)
{
  ...
  printf("%*s-@%s [%lux%lu]: {", space + 3, "", AH5_A_VALUE, (long unsigned) rational->dims[0],
         (long unsigned) rational->dims[1]);
